...
export default function DisputeDetail() {
  ...
  const pickMedia = async () => {
    ...
    if ((type === "image" && media.length >= 5) || (type === "video" && (videoCount >= 1 || photoCount > 4))) {
    ...
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView className="px-4 pt-4 pb-40">
        ...
        <View className="relative items-center justify-center mb-6">
          ...
          <Text className="text-xl font-semibold text-center">Detail Masalah</Text>
        </View>
        ...
        <Text className="text-xs text-gray-600 mb-1">
          Unggah maksimal <Text className="font-semibold">5 foto</Text> atau <Text className="font-semibold">4 foto + 1 video</Text>. Format: .jpg, .png, .mp4, .mov. Maks. 10 MB (foto), 50 MB (video).
        </Text>
        <TouchableOpacity onPress={() => setShowTipsModal(true)}>
          <Text className="text-xs text-blue-500 font-medium mb-3">Pelajari</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={pickMedia} className="w-16 h-16 border border-gray-400 rounded-xl items-center justify-center mb-4">
          <Text className="text-2xl text-gray-500">＋</Text>
        </TouchableOpacity>
        <View className="flex-row flex-wrap gap-3 mb-8">
          {media.map((item, idx) => (
            <View key={idx} className="w-20 h-20 bg-gray-200 rounded-xl overflow-hidden">
              {item.type === "image" ? (
        ...
        {/* Solusi */}
        <Text className="text-sm font-semibold text-black mb-2">Solusi apa yang kamu inginkan</Text>
        <Text className="text-xs text-gray-600 mb-3">Solusi bisa berubah, sesuai kesepakatan antara Seller, Buyer dan Tim Rekbr by BNI</Text>
        <TouchableOpacity
          className="flex-row justify-between items-center bg-white border border-gray-300 rounded-xl px-4 py-4"
          onPress={() => setShowModal(true)}
        >
          <Text className={`text-sm ${selectedSolution ? "text-black" : "text-gray-400"}`}>
        ...
        <View className="flex-1 justify-center items-center bg-black/40 px-6">
          <View className="bg-white rounded-2xl w-full px-6 pt-5 pb-5">
            <View className="flex-row items-start space-x-3 mb-7">
              <Image source={require("../../assets/icon-info-blue.png")} className="w-5 h-5 mt-1" resizeMode="contain" />
            ...
            <View className="flex-row justify-between">
              <TouchableOpacity onPress={() => setShowConfirmModal(false)} className="w-[48%] py-[14px] border border-gray-300 rounded-xl items-center">
                <Text className="text-[16px] font-semibold text-black">Kembali</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleConfirm} className="w-[48%] py-[14px] bg-blue-600 rounded-xl items-center">
                <Text className="text-[16px] font-semibold text-white">Konfirmasi</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View className="flex-1 justify-end">
          ...
          <View className="bg-white rounded-t-3xl px-6 pt-6 pb-10">
            ...
            {solutionOptions.map((item, index) => (
              <TouchableOpacity
                key={index}
                className={`mb-3 p-4 rounded-xl border ${selectedSolution === item.title ? "border-gray-800 bg-gray-50" : "border-gray-200"}`}
                onPress={() => {
      ...
      <Modal visible={showTipsModal} transparent animationType="slide">
        <View className="flex-1 justify-end">
          ...
          <View className="bg-white rounded-t-3xl pt-4 px-6 pb-6" style={{ maxHeight: "85%" }}>
            <View className="w-12 h-1.5 bg-gray-300 rounded-full self-center mb-6" />
            <Text className="text-lg font-semibold mb-4">Tips Upload Bukti</Text>
            {["Tampilkan kondisi barang sebelum kemasan dibuka",
              ...
              <View key={index} className="flex-row items-start mb-3">
                <Image source={require("../../assets/icon-check-green.png")} className="w-5 h-5 mt-0.5 mr-2" resizeMode="contain" />
                <Text className="text-sm text-black leading-snug flex-1">{tip}</Text>
              </View>
            ))}

            <View style={{ backgroundColor: "#F3F4F6" }} className="p-4 rounded-xl mt-4">
              <Text className="text-sm font-semibold text-black mb-2">Format yang didukung:</Text>
              <Text className="text-sm text-black leading-tight mb-1">
                • Maksimal <Text className="font-semibold">5 foto</Text> atau <Text className="font-semibold">4 foto dan 1 video</Text>
              </Text>
              <Text className="text-sm text-black leading-tight mb-1">
                • Format yang diterima adalah <Text className="font-semibold">.jpg, .png, .mp4, .mov</Text>
              </Text>
              <Text className="text-sm text-black leading-tight mb-1">
                • Ukuran maksimal foto adalah <Text className="font-semibold">10 MB per file</Text>
              </Text>
              <Text className="text-sm text-black leading-tight mb-1">
                • Ukuran maksimal video adalah <Text className="font-semibold">50 MB per file</Text>
              </Text>
              <Text className="text-sm text-black leading-tight mb-1">
                • Durasi video maksimal adalah <Text className="font-semibold">5 menit</Text>
              </Text>
              <Text className="text-sm text-black leading-tight">
                • Jika video terlalu besar, gunakan video compressor atau <Text className="font-semibold">upload video ke YouTube</Text> dan sertakan link di field alasan
              </Text>
            </View>
          </View>
        </View>

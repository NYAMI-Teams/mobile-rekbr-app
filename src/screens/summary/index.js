...
export default function TransactionSummaryScreen({ payload, bankData }) {
        ...
        <SafeAreaView className="flex-1 bg-white relative">
            ...
            >
                ...
                <View className="space-y-3 mb-6 mt-6">
                    ...
                    <Text className="text-lg font-semibold text-gray-900">{payload?.email}</Text>

                    <Text className="text-base text-gray-800 mt-4">Nama Barang</Text>
                    <Text className="text-lg font-semibold text-gray-900">{payload?.itemName}</Text>

                    <Text className="text-base text-gray-800 mt-4">Nominal Barang</Text>
                    <Text className="text-lg font-semibold text-gray-900">{formatCurrency(payload?.itemPrice)}</Text>
                </View>

                <View className="border-t border-gray-200 my-4" />

                {/* Biaya Tambahan */}
                {payload?.isInsurance && <View className="mb-4">
                    <View className="flex-row items-center mb-1">
                        <Text className="text-lg text-gray-800">Asuransi Pengiriman BNI Life</Text>
                        <Info size={14} color="#888" className="ml-1" />
                    </View>
                    <Text className="text-base font-medium text-gray-800">{formatCurrency(insuranceFee)}</Text>
                </View>}
                ...
                <View className="mb-6 mt-3">
                    ...
                    <Text className="text-base font-medium text-gray-800">{formatCurrency(serviceFee)}</Text>
                </View>
            </ScrollView>
            ...
            <View className="p-3 border-t-2 rounded-t-3xl border-x-2 border-gray-200 drop-shadow-xl items-center justify-between">
                <View className="flex-row justify-between items-center mb-4 w-full px-2">
                    ...
                    <Text className="text-xl font-bold text-gray-900">{formatCurrency(totalAmount)}</Text>
                </View>
                <PrimaryButton
